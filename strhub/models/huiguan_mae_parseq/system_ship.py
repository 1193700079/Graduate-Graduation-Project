...
class MAE_PARSeq(CrossEntropySystem):
    ...
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        if img_size[0] == 32 and img_size[1] == 128:
            # chkpt_dir = "pretrain_model/union14m/32x128_pretrain_union14m_vit_checkpoint_20.pth" #32*128 union14m
            # chkpt_dir = "pretrain_model/ship/32x128_pretrain_ship_tiny_checkpoint-1499.pth" #32*128 union14m
            chkpt_dir = "pretrain_model/ship/ship_32x128_4_768_checkpoint-1499.pth"
            checkpoint = torch.load(chkpt_dir, map_location='cpu')
            mae_model = getattr(models_mae, 'mae_vit_base_patch4')()

        elif img_size[0] == img_size[1] == 224:
            # chkpt_dir = "pretrain_model/ship/224x224_pretrain_ship_vit_checkpoint-1499.pth" #224*224 union14m
            # chkpt_dir = "pretrain_model/union14m/224x224_pretrain_union14m_vit_checkpoint-20.pth" #224*224 union14m
            # checkpoint = torch.load(chkpt_dir, map_location='cpu')
            mae_model = getattr(models_mae, 'mae_vit_base_patch16_224x224')()
            # codebook_mae_model = getattr(models_mae, 'mae_vit_base_patch16_224x224')()

        # mae_model.load_state_dict(checkpoint['model'], strict=False)
        self.encoder = mae_model
        # self.codebook_encoder = codebook_mae_model
        # self.gpt2_decoder = GPT2ForFixedLengthOutput(visual_feature_dim=768, output_length=50)
        embed_dim = 768
        ...
        if img_size[0] == 32 and img_size[1] == 128:
            ...
            # scale4 = 197 ** -0.5
            self.proj_to_197=   nn.Parameter(scale4 * torch.randn(77, 197))
    ...
    def encode(self, mae_img,clip_img,mask_ratio=0):
        # mae_img,clip_img = img
        # mae_img = img
        mae_memory,cls_tokens,encoder_features = self.encoder(mae_img, mask_ratio)
        ...
        if self.img_size[0] == 32 and self.img_size[1] == 128:
            text_features = text_features @ self.proj_to_257
        else:
            text_features = text_features @ self.proj_to_197   
        text_features = text_features.permute(0,2,1)
    ...
    def forward(self, mae_images,clip_images, max_length: Optional[int] = None) -> Tensor:
        ...
        memory = self.encode(mae_images,clip_images)
