...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def configure_optimizers(self):
        ...
        lr = lr_scale * self.lr #self.lr 0.0007
        optim = create_optimizer_v2(self, 'adamw', lr, self.weight_decay) # self.weight_decay : 0.0
        sched = OneCycleLR(optim, lr, self.trainer.estimated_stepping_batches, pct_start=self.warmup_pct,
                           cycle_momentum=False) #先增大学习率然后再增大
    ...
    def _eval_step(self, batch, validation: bool) -> Optional[STEP_OUTPUT]:
        ...
        confidence = 0
        label_length = 0
        if validation:  # 验证集  需要处理一下label 所有先调用forward_logits_loss
...
class CrossEntropySystem(BaseSystem):
    ...
    def forward_logits_loss(self, images: Tensor, labels: List[str]) -> Tuple[Tensor, Tensor, int]:
        ...
        logits = self.forward(images, max_len) # 验证集

        # if type(logits) == list:
        #     logits_list = logits
        #     loss_list = []
        #     loss_numel_list = []
        #     for logits_item in logits_list:
        #         loss = F.cross_entropy(logits_item.flatten(end_dim=1), targets.flatten(), ignore_index=self.pad_id)
        #         loss_numel = (targets != self.pad_id).sum()
        #         loss_list.append(loss)
        #         loss_numel_list.append(loss_numel)
            
        #     index = torch.tensor(loss_list).argmin().cpu()
        #     # print(index)
        #     loss = loss_list[index]
        #     loss_numel = loss_numel_list[index]
