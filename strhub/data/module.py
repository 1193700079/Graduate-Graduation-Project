...
class SceneTextDataModule(pl.LightningDataModule):
    TEST_BENCHMARK = ['IIIT5k', 'SVT','IC13_1015',  'IC15_1811','IC15_2077' ,'SVTP', 'CUTE80']
    TEST_NEW = ['ArT', 'COCOv1.4', 'Uber']
    TEST_UNION14M = ['artistic','contextless','curve','multi_oriented','multi_words','salient','general']
    TEST_SHIP = ['final_ship_0419']
    TEST_ALL = list(set(TEST_BENCHMARK+ TEST_NEW+TEST_UNION14M))
    ...
    def __init__(self, root_dir: str, train_dir: str, val_dir: str,img_size: Sequence[int], max_label_length: int,
        ...
        if os.path.isfile(charset_train):
            self.charset_train = readfile(charset_train) # path
            self.charset_test = readfile(charset_test) # path
    ...
    def train_dataset(self):
        if self._train_dataset is None:
            ...
            transform_clip = self.get_transform((224,224), self.augment, mean=self.mean, std=self.std)
            ...
            self._train_dataset = build_tree_dataset(root, self.charset_train, self.max_label_length,
                                                     ...
                                                     transform=transform,transformclip=transform_clip)
    ...
    def val_dataset(self):
        if self._val_dataset is None:
            ...
            transform_clip = self.get_transform((224,224), mean=self.mean, std=self.std)
    ...
    def test_dataloaders(self, subset):
        ...
        transform_clip = self.get_transform((224,224), self.augment, mean=self.mean, std=self.std)
        root = PurePath(self.root_dir, 'test/real')
        datasets = {s: LmdbDataset(str(root / s), self.charset_test, self.max_label_length,
                                   self.min_image_dim, self.remove_whitespace, self.normalize_unicode,
                                   transform=transform,transformclip=transform_clip) for s in subset}
